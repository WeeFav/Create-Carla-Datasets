...
def main(args):
    ...
    for epoch in range(args.max_epoch):
        ...
        for i, data_dict in enumerate(tqdm(train_dataloader)):
            """
            data_dict = {
                batched_pts:	        List of torch tensors of shape (N, 4) — raw LiDAR points per sample
                batched_gt_bboxes:	    List of tensors (M, 7) — 3D boxes per sample (center, size, yaw)
                batched_labels:	        List of tensors (M,) — class labels for each GT box
                batched_names:	        List of string lists — class names (e.g., ['Car', 'Pedestrian'])
                batched_difficulty:	    List of tensors — difficulty level per GT box
                batched_img_info:	    List of image info dictionaries  (e.g., shape, path, index)
                batched_calib_info:	    List of calibration dictionaries (e.g., P2, Tr_velo_to_cam)                
            }
            """
            if not args.no_cuda:
