...
def read_calib(file_path, extend_matrix=True):
    ...
    returns
    {
        'P0': np.ndarray of shape (4, 4) if extend_matrix else (3, 4),
        'P1': np.ndarray of shape (4, 4) if extend_matrix else (3, 4),
        'P2': np.ndarray of shape (4, 4) if extend_matrix else (3, 4),
        'P3': np.ndarray of shape (4, 4) if extend_matrix else (3, 4),
        'R0_rect': np.ndarray of shape (4, 4) if extend_matrix else (3, 3),
        'Tr_velo_to_cam': np.ndarray of shape (4, 4) if extend_matrix else (3, 4),
        'Tr_imu_to_velo': np.ndarray of shape (4, 4) if extend_matrix else (3, 4),
    }
    """
...
def read_label(file_path):
    ...
    returns
    {
        'name': np.ndarray of shape (N,), dtype=str            # Object class names (e.g., 'Car', 'Pedestrian')
        'truncated': np.ndarray of shape (N,), dtype=float32   # Truncation ratio [0.0 ~ 1.0]
        'occluded': np.ndarray of shape (N,), dtype=int32      # Occlusion state (0=visible, 1=partly, 2=largely, 3=unknown)
        'alpha': np.ndarray of shape (N,), dtype=float32       # Observation angle of object
        'bbox': np.ndarray of shape (N, 4), dtype=float32       # 2D bounding box [x1, y1, x2, y2]
        'dimensions': np.ndarray of shape (N, 3), dtype=float32 # 3D object size [l, h, w] (in camera coords)
        'location': np.ndarray of shape (N, 3), dtype=float32   # 3D object center location [x, y, z] (in camera coords)
        'rotation_y': np.ndarray of shape (N,), dtype=float32   # Rotation around Y-axis in camera coords
    }
    """
    with open(file_path, 'r') as f:
        lines = f.readlines()
